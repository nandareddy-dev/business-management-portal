      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black" style={{ color: '#111827' }}>Leads by City</h1>
              <p className="text-xs" style={{ color: '#9CA3AF' }}>
                {loading ? 'Loading…' : `${total} lead${total !== 1 ? 's' : ''}${isFiltered ? ' (filtered)' : ''} across ${rows.length} cit${rows.length !== 1 ? 'ies' : 'y'}`}
              </p>
            </div>
          </div>
                        ...
                        {r.leads.map(lead => (
                          <div key={lead.id} className="flex items-center gap-3 py-2.5" style={{ borderBottom: '1px solid #F1F1F5' }}>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10px]" style={{ color: '#9CA3AF' }}>{lead.source || '—'} {lead.pipeline_stage ? `· ${lead.pipeline_stage}` : ''}</p>
                            </div>
                            {lead.phone && (
