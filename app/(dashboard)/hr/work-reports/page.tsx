...
export default function CREDashboardPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    // ⚠️ FIXED (pagination): a plain `.limit(5000)` call does NOT guarantee 5000 rows —
    // Supabase/PostgREST enforces its own server-side `db-max-rows` setting (commonly 1000)
    // which silently caps the response *below* whatever `.limit()` you ask for, with no error.
    // For an active CRE, `lead_activities` in a single month can easily exceed that cap, so the
    // dashboard was quietly showing only the first page of activities (e.g. 396 calls shown vs
    // 3367 actually in the DB for July). Fix: page through with `.range()` until a page comes
    // back short, exactly like the existing `fetchAllLeads` helper already does for leads.
    const fetchAllActivities = async () => {
      const pageSize = 1000
      let from = 0
      let all: LeadActivity[] = []
      while (true) {
        ...
        if (error) { console.error('fetchAllActivities:', error); break }
        if (!data || data.length === 0) break
        all = all.concat(data as LeadActivity[])
    ...
    const [leadsRes, profilesRes, allActivities] = await Promise.all([
    ...
    const scopedByCompany = allActivities.filter(a => a.lead_id && leadIds.has(a.lead_id))
